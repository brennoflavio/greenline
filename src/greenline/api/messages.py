...
def _resolve_reply_context(
    chat_id: str,
    reply_context: ReplyContextRequest | dict[str, object] | None,
) -> dict[str, object] | None:
    if reply_context is None:
        return None

    if isinstance(reply_context, ReplyContextRequest):
        reply_id = reply_context.id.strip()
        if not reply_id:
            return None

        participant_raw = reply_context.participant
        participant_canonical = participant_raw
...
@crash_reporter
@dataclass_to_dict
def get_messages(request: GetMessagesRequest) -> MessagesResponse:
    next_cursor = ""
    has_more = False
    cursor_value = request.cursor or None
    with GreenlineKV() as kv:
        page_entries, _ = kv.get_partial_page_records(
            f"message:{request.chat_id}:",
            page_size=request.page_size + 1,
            cursor=cursor_value,
            reverse=True,
        )
        has_more = len(page_entries) > request.page_size
        entries = page_entries[: request.page_size]
...
def mark_messages_as_read(request: ChatIdRequest) -> SuccessResponse:
    ...
    try:
        rpc.clear_chat_notifications([request.chat_id])
    except Exception:
        pass
...
@crash_reporter
@dataclass_to_dict
def send_text_message(request: SendTextMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)
    validated_spans = validate_mention_spans(request.text, request.mention_spans)
    now = datetime.now()
...
@crash_reporter
@dataclass_to_dict
def edit_text_message(request: EditTextMessageRequest) -> SuccessResponse:
    normalized_text = request.text
    if normalized_text.strip() == "":
        return SuccessResponse(success=False, message="Message text cannot be empty")

    entry_key, entry = _get_message_entry_with_key(request.chat_id, request.message_id)
    ...
    reply_context = _resolve_message_reply_context(request.chat_id, entry)

    try:
        rpc = daemon_client()
        rpc.edit_message(request.chat_id, request.message_id, normalized_text, reply_context=reply_context)
...
@crash_reporter
@dataclass_to_dict
def delete_message(request: DeleteMessageRequest) -> SuccessResponse:
    entry_key, entry = _get_message_entry_with_key(request.chat_id, request.message_id)
    if entry_key is None or entry is None:
        return SuccessResponse(success=False, message="Message not found")
    ...
    try:
        daemon_client().delete_message(request.chat_id, request.message_id)
    except Exception as error:
        return SuccessResponse(success=False, message=str(error))
...
@crash_reporter
@dataclass_to_dict
def send_image_message(request: SendImageMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)

    cache_dir = os.path.join(get_cache_path(), "outgoing")
    os.makedirs(cache_dir, exist_ok=True)
    ext = os.path.splitext(request.file_path)[1] or ".jpg"
    cached_path = os.path.join(cache_dir, f"{request.temp_id or int(datetime.now().timestamp())}{ext}")
...
@crash_reporter
@dataclass_to_dict
def send_video_message(request: SendVideoMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)

    cache_dir = os.path.join(get_cache_path(), "outgoing")
    os.makedirs(cache_dir, exist_ok=True)
    ext = os.path.splitext(request.file_path)[1] or ".mp4"
    cached_path = os.path.join(cache_dir, f"{request.temp_id or int(datetime.now().timestamp())}{ext}")
    shutil.copy2(request.file_path, cached_path)
...
@crash_reporter
@dataclass_to_dict
def send_audio_message(request: SendAudioMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)
    duration_value = max(0, request.duration_seconds)
    duration = _format_duration(duration_value)
    now = datetime.now()

    cache_dir = os.path.join(get_cache_path(), "outgoing")
    os.makedirs(cache_dir, exist_ok=True)
    ext = os.path.splitext(request.file_path)[1] or ".ogg"
    cached_path = os.path.join(cache_dir, f"{request.temp_id or int(now.timestamp())}{ext}")

    try:
        if os.path.abspath(request.file_path) != os.path.abspath(cached_path):
            shutil.move(request.file_path, cached_path)
    except Exception as error:
        ...
        failed_msg = Message(
            ...
            mimetype=_guess_audio_mimetype(request.file_path),
            send_status="failed",
            temp_id=failed_id,
        )
...
@crash_reporter
@dataclass_to_dict
def send_sticker_message(request: SendStickerMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)

    cache_dir = os.path.join(get_cache_path(), "outgoing")
    os.makedirs(cache_dir, exist_ok=True)
    ext = os.path.splitext(request.file_path)[1] or ".webp"
    cached_path = os.path.join(cache_dir, f"{request.temp_id or int(datetime.now().timestamp())}{ext}")
    shutil.copy2(request.file_path, cached_path)
...
@crash_reporter
@dataclass_to_dict
def send_contact_message(request: SendContactMessageRequest) -> SuccessResponse:
    resolved_reply_context = _resolve_reply_context(request.chat_id, request.reply_context)
    ...
    cache_dir = os.path.join(get_cache_path(), "outgoing")
    os.makedirs(cache_dir, exist_ok=True)
    ext = _guess_contact_extension(request.file_path)
    cached_path = os.path.join(cache_dir, f"{request.temp_id or int(datetime.now().timestamp())}{ext}")
    shutil.copy2(request.file_path, cached_path)
...
@crash_reporter
@dataclass_to_dict
def download_media(request: DownloadMediaRequest) -> DownloadMediaResponse:
    entry_key, entry = _get_message_entry_with_key(request.chat_id, request.message_id)
    if entry_key is None or entry is None:
        return DownloadMediaResponse(success=False, media_path="", message="Message not found")

    if request.media_type not in _MEDIA_TYPE_MAP:
    ...
    if media.media_type != request.media_type:
